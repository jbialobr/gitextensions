    ...
    public partial class FormApplyPatch : GitExtensionsForm
    {
        ...
        private void Apply_Click(object sender, EventArgs e)
        {
            ...
            if (PatchFileMode.Checked)
                ...
                {
                    new FormProcess(GitCommandHelpers.PatchCmdIgnoreWhitespace(PatchFile.Text), PerFormSettingsName()).ShowDialog(this);
                }
                else
                {
                    new FormProcess(GitCommandHelpers.PatchCmd(PatchFile.Text), PerFormSettingsName()).ShowDialog(this);
                }
            else
                if (IgnoreWhitespace.Checked)
                {
                    new FormProcess(GitCommandHelpers.PatchDirCmdIgnoreWhitespace(PatchDir.Text), PerFormSettingsName()).ShowDialog(this);
                }
                else
                {
                    new FormProcess(GitCommandHelpers.PatchDirCmd(PatchDir.Text), PerFormSettingsName()).ShowDialog(this);
                }
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            new FormProcess(GitCommandHelpers.SkipCmd(), PerFormSettingsName()+"Skip").ShowDialog(this);
            EnableButtons();
            Cursor.Current = Cursors.Default;
        }
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            new FormProcess(GitCommandHelpers.ResolvedCmd(), PerFormSettingsName()+"Resolved").ShowDialog(this);
            EnableButtons();
            Cursor.Current = Cursors.Default;
        }
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            new FormProcess(GitCommandHelpers.AbortCmd(), PerFormSettingsName() + "Abort").ShowDialog(this);
            EnableButtons();
            Cursor.Current = Cursors.Default;
        }
