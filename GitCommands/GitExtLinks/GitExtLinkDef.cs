...
namespace GitCommands.GitExtLinks
{
    public class GitExtLinkDef : SimpleStructured
    {
        ...
        public IEnumerable<GitExtLink> Parse(GitRevision revision, IGitRemoteController remoteController)
        {
            ...
            foreach (Match remoteMatch in remoteMatches)
            {
                IEnumerable<GitExtLink> links = ParseRevision(remoteMatch, revision);

                foreach (GitExtLink link in links)
                {
                    yield return link;
                }
            }
        ...
        public IEnumerable<Match> ParseRemotes(IGitRemoteController remoteController)
        {
            ...
            {
                ISet<string> remoteUrls = new SortedSet<string>();
        ...
        private IEnumerable<GitRemote> GetMatchingRemotes(IEnumerable<GitRemote> remotes)
        {
            ...
            matchingRemotes = matchingRemotes.OrderBy(r => UseRemotesPattern.IndexOf(r.Name, StringComparison.OrdinalIgnoreCase));
    ...
    public class GitExtLinkFormat : SimpleStructured
    {
        ...
        public GitExtLink ToGitExtLink(Match remoteMatch, Match revisionMatch, GitRevision revision)
        {
            ...
            AddGroups(remoteMatch, groups);
            AddGroups(revisionMatch, groups);
        ...
        private void AddGroups(Match match, List<string> groups)
        {
            if (match != null)
            {
